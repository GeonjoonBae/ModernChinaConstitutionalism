...
result_file = f"bcc_{start_year}-{end_year}_{output_file_keyword}_similarity_tfidf.csv"
...
plt.title(f"Yearly Similarity (TF-IDF, {output_file_keyword}, {start_year}-{end_year})")
plt.tight_layout()
img_file = f"bcc_{start_year}-{end_year}_{output_file_keyword}_similarity_tfidf.png"
plt.savefig(os.path.join(output_directory, img_file), dpi=200)
plt.close()
print(f"완료: {result_file}, {img_file}")

# 표준편차 결과 저장
std_file = f"bcc_{start_year}-{end_year}_{output_file_keyword}_similarity_tfidf_std.csv"
