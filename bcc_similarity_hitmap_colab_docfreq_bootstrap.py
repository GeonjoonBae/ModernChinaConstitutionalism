...
xml_filepath = r"bcc_corpusdata_canzheng_1912-1945.xml"
output_directory = '.'
...
for _ in range(bootstrap):
    # 각 연도별, 샘플 entry별 토큰화 후 리스트 저장
    docs_by_year = []
    for year in years:
        docs = entries_by_year[year]
        sample = random.sample(docs, sample_size) if len(docs) > sample_size else docs
        doc_tokens = [" ".join(tokenize(doc)) for doc in sample]
        docs_by_year.append(doc_tokens)

    # 각 연도의 어휘 문서 빈도 벡터화
    # 모든 연도의 문장 모아서 어휘 집합 생성
    all_docs = [doc for doclist in docs_by_year for doc in doclist]
    vec = CountVectorizer()
    vec.fit(all_docs)
    # 연도별로, 각 단어가 entry 중 몇 개 문서에 등장하는지(문서 빈도)
    year_dfs = []
    for doclist in docs_by_year:
        mat = vec.transform(doclist).toarray()
        dfvec = (mat > 0).sum(axis=0) / mat.shape[0]
        year_dfs.append(dfvec)
    sim_mat = cosine_similarity(year_dfs)
...
result_file = f"bcc_{start_year}-{end_year}_{output_file_keyword}_similarity_docfreq.csv"
...
plt.title(f"Yearly Similarity (DocFreq, {output_file_keyword}, {start_year}-{end_year})")
plt.tight_layout()
img_file = f"bcc_{start_year}-{end_year}_{output_file_keyword}_similarity_docfreq.png"
plt.savefig(os.path.join(output_directory, img_file), dpi=200)
plt.close()
print(f"완료: {result_file}, {img_file}")

# 표준편차 결과 저장
std_file = f"bcc_{start_year}-{end_year}_{output_file_keyword}_similarity_docfreq_std.csv"
std_df.to_csv(os.path.join(output_directory, std_file), encoding='utf-8-sig')

# CSV 파일 다운로드
files.download(os.path.join(output_directory, result_file))   # 유사도 매트릭스
files.download(os.path.join(output_directory, std_file))      # 표준편차 결과

# PNG(히트맵) 파일 다운로드
files.download(os.path.join(output_directory, img_file))